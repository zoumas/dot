...
hl.config({
	general = {
		...
		col = {
			active_border = { colors = { "rgba(f9e2afee)", "rgba(fab387ee)" }, angle = 45 },
...
-- Smart gaps: no gaps/border when a workspace has exactly one tiled window
...
hl.bind(mainMod .. " + SHIFT + V", hl.dsp.exec_cmd("cliphist list | hyprlauncher -m | cliphist decode | wl-copy"))
...
-- Swap window position with mainMod + ALT + vim keys
...
-- Screenshots with grim + slurp: region to clipboard + file, full screen with SHIFT
hl.bind(
	mainMod .. " + S",
	hl.dsp.exec_cmd(
		'mkdir -p ~/Pictures/Screenshots && grim -g "$(slurp)" - | tee ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png | wl-copy && notify-send "Screenshot" "Region copied to clipboard"'
	)
)
hl.bind(
	mainMod .. " + SHIFT + S",
	hl.dsp.exec_cmd(
		'mkdir -p ~/Pictures/Screenshots && grim - | tee ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png | wl-copy && notify-send "Screenshot" "Full screen copied to clipboard"'
	)
)
...
-- Media controls on mainMod + F8-F10 (no dedicated media keys on this keyboard)
hl.bind(mainMod .. " + F8", hl.dsp.exec_cmd("playerctl -p spotify play-pause"))
hl.bind(mainMod .. " + F9", hl.dsp.exec_cmd("playerctl -p spotify previous"))
hl.bind(mainMod .. " + F10", hl.dsp.exec_cmd("playerctl -p spotify next"))

-- Audio controls on mainMod + F1-F4 (no dedicated media keys on this keyboard)
...
-- Laptop LCD brightness on mainMod + F5/F6
hl.bind(mainMod .. " + F5", hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%-"), { repeating = true })
hl.bind(mainMod .. " + F6", hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%+"), { repeating = true })
